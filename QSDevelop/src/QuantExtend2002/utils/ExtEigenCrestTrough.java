...
public class ExtEigenCrestTrough {
	...
	public static void test(DAKLines kLines, int iBegin, int iEnd) {
		...
		for (int i = iBegin; i<iEnd; i++) {
			...
			boolean bck = check(kLines, i);
			PoiList.add(new CurvePoint(i,kLines.get(i).close, bck));
		}
...
//				kLines.get(iCheck).date, kLines.get(iIdxLowLeft).date, kLines.get(iIdxLowRight).date);
		...
		//判断：趋势要求：上升阶段中间3天均值（不包含首尾，如果天数较少要时候少于3天） 介于 上升开始3天收盘价平均值（不包含top天） 和 上升结束3天收盘价平均值（不包含lowLeft） 之间， 下降阶段同理
		{
			int addCount = 0;
		...
		CLog.output(TAG, "ExtEigenCrestTrough.check TOPdate:%s OK!", kLines.get(iCheck).date);
